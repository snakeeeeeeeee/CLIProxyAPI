...
1. 在输入框中一行填写一个自有 SessionKey，单批最多 100 条。
2. 设置登录并发，范围 1-5，默认 2。
3. 选择是否“绑定独立代理 IP”。默认开启；开启时每个 SessionKey 需要一个健康且未绑定的代理。
4. 点击“开始批量登录”，输入框会在后端接受任务后立即清空。
5. 在弹窗中查看新增、更新、失败、代理模式和逐条结果。
6. 关闭弹窗不会取消任务；重新打开或刷新页面后会恢复当前任务状态。

代理规则：

- 开启“绑定独立代理 IP”时，只选择 `enabled + healthy + 未绑定 + 未预留` 的代理。
- 绑定模式按输入顺序预留，每个 SessionKey 固定一个代理；代理不足时，已有代理的条目继续执行，其余显示 `no_proxy`。
- 绑定模式失败不会自动更换代理；登录成功后账号绑定本次代理，已有账号更新 token 并换绑代理，旧代理自动释放。
- 关闭绑定时不预留或占用代理池，不会因为没有健康代理而返回 `no_proxy`。
- 未绑定登录优先使用 `config.yaml` 的全局代理；没有全局代理时使用服务器直连。
- 未绑定模式新增的账号保持无代理；更新已有账号时只更新 OAuth 凭据，不解除该账号原有代理绑定。
...
- `success`：新增账号。
- `updated`：更新已有账号 token；绑定模式会换绑本次代理，未绑定模式保留原绑定。
- `duplicate_input`：同一批输入重复。
- `duplicate_account`：不同 SessionKey 最终对应同一账号。
- `no_proxy`：绑定模式下没有可分配的健康空闲代理。
...
- 控制台需先勾选账号，再点击“导出所选”或批量栏中的“导出 OAuth”；按钮会显示当前勾选数量。
- Management API 仍支持 `ids=[]` 导出指定池的全部账号，便于脚本备份。
