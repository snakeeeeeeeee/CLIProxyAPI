...
func TestPoolRouterLimitsConcurrencyAndReleases(t *testing.T) {
	...
	lease, ok := router.tryAcquire("auth-1", "claude-opus", policy, time.Now())
	if !ok || lease == nil {
		t.Fatal("first acquire should succeed")
	}
	if _, ok := router.tryAcquire("auth-1", "claude-opus", policy, time.Now()); ok {
...
func TestPoolRouterLimitsRPM(t *testing.T) {
	...
	now := time.Now()
	lease, ok := router.tryAcquire("auth-1", "claude-opus", policy, now)
	if !ok || lease == nil {
		t.Fatal("first acquire should succeed")
	}
	lease.Release()
	if _, ok := router.tryAcquire("auth-1", "claude-opus", policy, now.Add(time.Second)); ok {
		t.Fatal("second acquire should be blocked by rpm")
	}
	if _, ok := router.tryAcquire("auth-1", "claude-opus", policy, now.Add(routeWindow+time.Second)); !ok {
...
func TestPoolRouterCoolsDown529PerAuthModel(t *testing.T) {
	...
	if _, ok := router.tryAcquire("auth-1", "claude-sonnet", policy, now.Add(time.Second)); !ok {
		t.Fatal("different model should not be blocked")
	}
	if _, ok := router.tryAcquire("auth-2", "claude-opus", policy, now.Add(time.Second)); !ok {
...
func TestPoolRouterLimitsRPMAndConcurrencyPerAccount(t *testing.T) {
	...
	lease.Release()
	if _, ok := router.tryAcquire("auth-1", "claude-sonnet", policy, now.Add(2*time.Second)); ok {
		t.Fatal("different model should still share account rpm")
	}
	if _, ok := router.tryAcquire("auth-1", "claude-sonnet", policy, now.Add(routeWindow+time.Second)); !ok {
