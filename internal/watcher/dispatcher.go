...
func (w *Watcher) refreshAuthState(force bool) {
	...
	auths := snapshotCoreAuthsFunc(cfg, configPath, authDir, parser)
	w.clientsMutex.Lock()
...
func snapshotCoreAuths(cfg *config.Config, configPath string, authDir string, parser synthesizer.PluginAuthParser) []*coreauth.Auth {
