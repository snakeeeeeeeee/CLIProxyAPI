...
func TestAuthFileEventsDoNotInvokeSnapshotCoreAuths(t *testing.T) {
	...
	snapshotCoreAuthsFunc = func(cfg *config.Config, configPath string, authDir string, parser synthesizer.PluginAuthParser) []*coreauth.Auth {
		atomic.AddInt32(&snapshotCalls, 1)
		return origSnapshot(cfg, configPath, authDir, parser)
	}
