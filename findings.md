...
- The current title-helper recognizer does not require non-empty tools, so the implementation needs a regression fixture rather than a new request-shape heuristic.
- Existing account-pool auths inherit global/environment proxy settings whenever `ProxyURL` remains empty. Setting unbound stored auths and overlays to explicit `direct` is the smallest shared enforcement point for inference, count-tokens, refresh, quota, and management tests.
- Current quota scheduling is enabled by default at five minutes and new registrations trigger an immediate active usage request. Both conflict with the approved passive-first policy and must be replaced by explicit modes and delayed scheduled collection.
- Current proxy health performs a configurable target request and does not parse/store the observed exit IP. The approved implementation will collapse this into one ipify JSON request per check.
- `EffectiveAccountQuota` initially used `quotaDefaultInterval.String()`, which serializes as `30m0s` and is outside the public `15m/30m/1h` enum. Canonical API strings must not be generated through `time.Duration.String()`.
- `singleflight.Do` marks both the closure owner and waiting callers as shared. Correct `network_performed/cached` response metadata requires a caller-local closure-ownership flag, not the returned `shared` boolean.
- The first strict-proxy pass set missing bindings to `invalid://bound-proxy`, but stored and overlaid auths still retained a concrete URL when a bound proxy was disabled or unhealthy. The final shared resolver returns a concrete URL only for present, enabled, non-unhealthy, parseable proxy resources.
- OAuth token exchange had the Axios-compatible Header tuple, while refresh still sent generic JSON Accept with no User-Agent. Both now use the same `oauth-token` Header helper.
- The active quota refresh endpoint originally returned `200 + warning` in disabled mode, and recheck omitted result-reuse metadata. The management contract now returns HTTP 409 for disabled active collection and the same network/cache timestamps for both manual actions.

## sub2api Anthropic Quota Collection Comparison (2026-07-26)

- The inspected checkout is `/Users/zhangyu/code/myProject/sub2api`, upstream `Wei-Shaw/sub2api`, commit `2730c1c43b29be003925b033f3f9e645e726bb8c`, version `0.1.165`. The similarly named checkout under `supertoken-projects` is older and was not used for conclusions.
- sub2api has no Anthropic background worker that periodically scans all accounts and calls `/api/oauth/usage`. The only production call path into `AccountUsageService.GetUsage` is the admin account usage handler. Inference responses separately feed passive rate-limit Header observations.
- Anthropic cards are passive-first. On mount and on the account-list manual refresh signal, the frontend requests `source=passive`, which reads stored account fields and does not contact Anthropic. Only the dedicated active-query control requests `source=active&force=true`.
- Active Anthropic queries cache successful `/api/oauth/usage` responses for 3 minutes and errors for 1 minute, deduplicate concurrent requests per account with `singleflight`, and add 0-800ms jitter before a cache miss. The nearby comment still says 10 minutes and is stale.
- The handler parses `force=true`, but the Anthropic branch never uses `forceProbe`; only OpenAI and Grok receive it. Therefore the active-query control does not actually bypass the 3-minute Anthropic server cache. This is a sub2api defect, not a behavior to copy.
- The frontend has another 5-minute in-memory usage cache for normal loads. Its dedicated active-query path bypasses that frontend cache, but it can still receive the backend's cached 3-minute response because of the ignored force flag.
- One active request is `GET https://api.anthropic.com/api/oauth/usage` with Bearer OAuth, `anthropic-beta: oauth-2025-04-20`, JSON Accept/Content-Type, and an account-derived User-Agent when the Redis fingerprint exists. The hard fallback is the stale-looking `claude-code/2.1.7` value.
- The active request uses the account's bound proxy. If account TLS fingerprinting is enabled, it also uses the account-resolved TLS Profile and an account-isolated upstream client; otherwise it uses the normal Go HTTP client with the same proxy and a 30-second request timeout.
- The usage request itself reads the currently stored access token and does not refresh before the call or refresh-and-retry after a 401. sub2api instead has an independent token refresh worker enabled by default, scanning every 5 minutes and refreshing within 30 minutes of expiry. Refresh also uses the account proxy. A race or disabled worker can still make a manual usage query fail on a stale token.
- Active parsing supports shared `five_hour`, shared `seven_day`, `seven_day_sonnet`, and Fable through `seven_day_overage_included`. It has no `seven_day_opus` field and no dynamic `limits[]` model parser.
- Passive inference Header sampling stores 5h, 7d, and `7d_oi`/Fable utilization/reset only. It does not store independent Sonnet or Opus passive windows.
- Active results are synchronized back into passive storage for 5h, 7d, and Fable. If active Fable is absent, passive `7d_oi` is used as a fallback. Sonnet is not synchronized to passive storage, Opus is absent, and passive-window construction does not reject stale observations; an expired reset only clamps remaining seconds to zero while the old utilization can remain visible.
- Compared with sub2api, CLIProxyAPI has broader model coverage: active parsing includes Sonnet, Opus, Fable, and dynamic model limits, while passive Header normalization is more generic. sub2api's advantage is lower unsolicited traffic and explicit cache-collapse controls, not more accurate model-level quota data.
- CLIProxyAPI already uses per-account singleflight and configured jitter for background scheduling, but its active OAuth due clock currently shares timestamps advanced by inference success and passive Header merges. Busy accounts can postpone active polling indefinitely. The recommended correction is an independent OAuth-probe clock based on the latest probe attempt, not removal of background polling.
- Recommended policy: retain a low-frequency active account probe (one account-level request, not one request per model), keep passive Header updates, preserve all fresh passive model windows when active responses omit them, and use singleflight plus bounded jitter/backoff. UI manual refresh should explicitly bypass any short success cache if a cache is later added.

## Claude Code 2.1.220, Heartbeats, And Quota Collection (2026-07-26)

- The local and npm-latest Claude Code version is 2.1.220. The available credential is injected by user settings together with a custom Base URL; disabling those settings reports no independent login. Captures must therefore be labeled custom-Base-URL evidence, not direct first-party OAuth evidence.
- A record-only 2.1.220 invocation generated two immediate `/v1/messages?beta=true` requests: a Haiku `generate_session_title` helper and the main Sonnet SDK request. Both used HTTP/1.1, the same ordered Header tuple, `claude-cli/2.1.220 (external, sdk-cli)`, Stainless JS 0.94.0 on MacOS/arm64 Node v26.3.0, retry count 0, timeout 600, and matching Session Header/body metadata.
- The billing suffix is request-kind-specific (`2.1.220.6cb` for title and `2.1.220.032` for the main request), so it must remain algorithmic rather than frozen to one observed suffix.
- The main request carried three `cache_control: {type: ephemeral}` objects with no explicit `ttl`. This sample does not support the old claim that current Claude Code explicitly sends `ttl: 1h`; under documented prompt-caching semantics, omitted TTL is normally 5 minutes. Credential/feature-flag differences remain possible and require direct first-party evidence before changing the compatibility baseline.
- An interactive 2.1.220 process remained idle for 70 seconds. The recorder received no HTTP request. Debug logged only that third-party telemetry was disabled and a local telemetry event was dropped. This is direct evidence against a universal every-few-seconds Messages heartbeat in this configuration.
- Official documentation distinguishes optional OpenTelemetry export intervals from inference traffic: metrics default to 60 seconds and logs/traces to 5 seconds only when telemetry/exporters are enabled. Binary heartbeat strings appear tied to PTY, transcript, remote-control, or session keepalive paths and are not evidence of periodic Messages inference.
- Current quota maintenance is account-level active OAuth polling, not model-inference polling: a 15-second scheduler scan refreshes only due accounts; default due time is five minutes plus deterministic +/-20% jitter and `account-quota.interval` is authoritative.
- Active `/api/oauth/usage` parsing can populate shared 5h/7d, Sonnet, Opus, Fable (`seven_day_overage_included`), and model-scoped `limits[]` windows in one account request. It does not need to call each model.
- Passive quota collection runs after real inference and merges any unified rate-limit Headers actually returned. Shared 5h/7d evidence applies across model families; a Sonnet/Opus/Fable request does not guarantee a model-specific window unless Anthropic returns the corresponding Header.
- The active merge explicitly preserves a fresh passive Fable fallback when OAuth usage omits Fable, but it does not apply the same preservation rule to fresh passive Sonnet and Opus windows. This should be generalized to all model families before claiming symmetric active/passive refresh behavior.
- The configured five-minute active cadence is not currently strict. `accountQuotaRefreshDue` treats the newest of health-check and quota `CheckedAt` timestamps as the scheduling reference; successful inference and response-Header merges both advance those timestamps and the shared `next_health_check_at`. A continuously busy account can therefore postpone `/api/oauth/usage` indefinitely. Active quota polling needs an independent due clock based on the latest OAuth probe attempt (the persisted safe `AccountQuotaProbe.RequestedAt` is already available), while passive observations and inference health must not reset it.
- Official Claude Code documentation confirms that OTel export intervals are unrelated to Messages calls: OTel must be enabled explicitly, metrics default to 60 seconds, and logs/traces default to 5 seconds to a configured collector. Official changelog references to heartbeats are specifically remote-session worker keepalives or progress updates during long-running tools, not proof of a local CLI inference heartbeat.
- Official prompt-caching documentation says `cache_control: {type: ephemeral}` defaults to 5 minutes and 1 hour requires `ttl: 1h`. The local 2.1.220 main request matches that documented 5-minute form. The current account-pool forced 1h rewrite is an intentional gateway policy, not a verified Claude Code 2.1.220 default.
- The external review's major code findings are reproducible: token refresh currently omits a User-Agent and uses the generic Chrome/H2 token transport; proxy health defaults to a Go-client GET against the Anthropic root; trace classification rejects legitimate tool-less title helpers that the runtime classifier accepts; mimic timeout is pinned to 600. The correct refresh fingerprint is still unknown without a real first-party token-refresh capture, so it should not be guessed from inference traffic.
- The current billing fingerprint algorithm reproduces the observed 2.1.220 main-request suffix: `Reply with exactly OK.` plus version 2.1.220 computes `032`, matching the recorder. This supports keeping the algorithmic fixture approach rather than hard-coding one suffix.
- Recommended version policy: freeze a reviewed compatibility profile revision and update it only after a compatibility trigger (upstream rejection, API field change, meaningful Claude Code protocol change, or scheduled audit). Normal users run older patch versions, so chasing every npm patch adds churn without establishing authenticity. Credential-specific and request-kind-specific behavior must remain separate from the frozen common transport tuple.
